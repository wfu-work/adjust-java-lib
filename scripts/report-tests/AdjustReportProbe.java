...
public class AdjustReportProbe {
  ...
  static ENUNetworkProblem network(int n) {
    ...
    for(int i=1;i<n;i++) {
      ...
      for(int j=0;j<2;j++) {double e=(j==0?1:-1)*.01;baselines.add(baseline("L"+i+"_"+j,"S0","S"+i,new ENU(i*10+e,i*2+e,i*.5+e),Matrix3.fromStdDev(.02,.02,.03)));}
    }
  ...
  public static void main(String[] args) throws Exception {
    ...
    test("weighted_analytic",()->{
      ...
      require(r.getDiagnostics().isConverged()&&r.getDiagnostics().isStationCovarianceAvailable(),"diagnostics");
    });
    ...
    test("robust_outlier",()->{
      ENUNetworkProblem p=weighted();List<ENUNetworkProblem.Baseline> bs=new ArrayList<>();for(int i=0;i<10;i++)bs.add(baseline("G"+i,"A","B",new ENU(10+(i%3-1)*.01,2+(i%3-1)*.01,.5),Matrix3.fromStdDev(.02,.02,.02)));bs.add(baseline("OUT","A","B",new ENU(15,7,5.5),Matrix3.fromStdDev(.02,.02,.02)));p.setBaselines(bs);
      ENUNetworkResult plain=SDK.startAdjust(p),robust=SDK.startAdjust(p,AdjustOptions.builder().robust(true).build());results.put("robust",Map.of("plain",plain,"huber",robust));require(robust.getBaselines().get(10).getWeight()<1,"outlier not downweighted");require(Math.abs(robust.getStations().get(1).getPosition().getEast()-10)<Math.abs(plain.getStations().get(1).getPosition().getEast()-10),"robust did not reduce coordinate error");require(robust.getDiagnostics().getGlobalTest()==null,"global test after downweight");
    });
